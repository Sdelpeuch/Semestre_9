...
void process(const char *sename, const char *imsname, const char *imdname) {
    ...
    clock_t start, end;
    double duration_sec;
    cout << INFO << "Applying CLosing to \"" << imsname << "\" with SE sharp \"" << sename << "\"..." << endl;
    start = clock();
    mm(se, ims, imd, &maximum); // dilatation ...
    mm(se, imd, imd2, &minimum); // ... puis erosion
    end = clock();
    duration_sec = double(end - start) / CLOCKS_PER_SEC;
    cout << INFO << "Time elapsed while applying Closing : " << duration_sec << "s" << endl;
